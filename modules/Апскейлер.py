# modules/upscaler.py — Облачный нейро-апскейлер v1.1 (Исправленная сигнатура API)
import os
import asyncio
import logging
import tempfile
...
def sync_upscale_hf(input_path: str, scale: int = 2, face_enhance: bool = True) -> str:
    """Синхронный вызов спейса Hugging Face со строгим соблюдением 6 аргументов"""
    from gradio_client import Client, handle_file

    client = Client("sczhou/CodeFormer")
    
    # 1. Подготовка входного фото: конвертируем в чистый RGB JPEG (защита от краша прозрачности RGBA)
    with Image.open(input_path) as img:
        img_rgb = img.convert("RGB")
        clean_input_path = str(Path(input_path).with_suffix(".jpg"))
        img_rgb.save(clean_input_path, format="JPEG", quality=98)

    # 2. Передаем ВСЕ 6 АРГУМЕНТОВ в строгом порядке, который требует модель:
    # 1: image (файл)
    # 2: face_align (выравнивание лица, True)
    # 3: background_enhance (Real-ESRGAN для фона, True)
    # 4: face_upsample (восстановление лиц, True/False)
    # 5: upscale (масштаб: 2 или 4)
    # 6: codeformer_fidelity (баланс резкости: 0.6)
    result = client.predict(
        handle_file(clean_input_path),  # 1. image
        True,                           # 2. face_align
        True,                           # 3. background_enhance
        face_enhance,                   # 4. face_upsample
        int(scale),                     # 5. upscale
        0.6,                            # 6. codeformer_fidelity
        fn_index=0
    )
...
def register(client, bot=None):
    ...
    async def upscale_handler(event):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            in_file = await reply_msg.download_media(file=tmp_path / "input_raw")
            if not in_file:
            ...
            try:
                ...
                await status.edit(
                    f"🎨 `Нейросеть восстанавливает детали...`\n"
                    f"• Исходный размер: `{orig_w}x{orig_h}`\n"
                    f"• Множитель: `{scale}x` {'(с реконструкцией лиц)' if face_enhance else ''}"
                )

                # Запуск тяжелого запроса в отдельном потоке
                ...
                if not out_path or not os.path.exists(out_path):
                    return await status.edit("❌ Сервер вернул пустой результат. Попробуйте еще раз.")
                ...
                caption = (
                    ...
                    f"• Режим: **{scale}x** (CodeFormer + Real-ESRGAN)"
                )
                ...
                err_text = str(e)
                if "queue" in err_text.lower() or "loading" in err_text.lower():
                    await status.edit("⏳ Очередь на видеокарту. Повторите попытку через 10 секунд.")
                else:
